...
public class RefereeDecisionController implements CountdownTimerListener, IDecisionController {
    ...
    private final static int DEFAULT_RESET_DISPLAY_DELAY = 5000;
    private int resetDisplayDelay = DEFAULT_RESET_DISPLAY_DELAY;

    /**
     * 3 seconds to change decision (after all refs have selected)
     */
    private final static int DEFAULT_DECISION_REVERSAL_DELAY = 3000;
    private int decisionReversalDelay = DEFAULT_DECISION_REVERSAL_DELAY;

    /**
     * Time before displaying decision once all referees have pressed.
     */
    private final static int DEFAULT_DECISION_DISPLAY_DELAY = 1000;
    private int decisionDisplayDelay = DEFAULT_DECISION_DISPLAY_DELAY;
    ...
    @Override
    public void initDownSignal() {
        final Platform platform = groupData.getPlatform();
        try {
            if (platform != null) {
                logger.debug("platform {}, {}",platform.getName(), System.identityHashCode(platform));
                Mixer mixer = platform.getMixer();
                if (mixer != null) {
                    downSignal = new Tone(mixer, 1100, 1200, 1.0);
                    logger.debug("platform {}, mixer = {}, {}",platform.getName(),mixer.getMixerInfo().getName(), System.identityHashCode(platform));
                } else {
                    logger.debug("no sound for platform {} {}", platform.getName(), System.identityHashCode(platform));
                }
            }
        } catch (Exception e) {
//            LoggerUtils.errorException(logger, e);
            logger.debug("{}: no sound for platform {} {}", e, platform.getName(), System.identityHashCode(platform));
        }
    }
    ...
    private void scheduleReset() {
        ...
        }, decisionReversalDelay + resetDisplayDelay);
    }
    ...
    public int getDecisionReversalDelay() {
        return decisionReversalDelay;
    }

    public void setDecisionReversalDelay(int d) {
        decisionReversalDelay = d;
    }

    public void resetDecisionReversalDelay() {
        decisionReversalDelay = DEFAULT_DECISION_REVERSAL_DELAY;
    }

    public int getDecisionDisplayDelay() {
        return decisionDisplayDelay;
    }

    public void setDecisionDisplayDelay(int d) {
        decisionDisplayDelay = d;
    }

    public void resetDecisionDisplayDelay() {
        decisionDisplayDelay = DEFAULT_DECISION_DISPLAY_DELAY;
    }

    public int getResetDisplayDelay() {
        return resetDisplayDelay;
    }

    public void setResetDisplayDelay(int d) {
        resetDisplayDelay = d;
    }

    public void resetResetDisplayDelay() {
        resetDisplayDelay = DEFAULT_RESET_DISPLAY_DELAY;
    }
}
