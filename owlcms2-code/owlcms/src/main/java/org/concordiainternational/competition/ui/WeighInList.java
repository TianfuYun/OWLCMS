...
public class WeighInList extends LifterHbnList implements ApplicationView, Bookmarkable, Notifyable {
    ...
    public WeighInList(boolean initFromFragment, String fragment, String viewName, boolean registration, CompetitionApplication app) {
        super(app, Messages.getString("WeighInList.title", CompetitionApplication.getCurrentLocale())); //$NON-NLS-1$
        if (initFromFragment) {
    ...
    protected void addGeneratedColumns() {
        ...
        table.addGeneratedColumn("missingKg", new CommonColumnGenerator(app)); //$NON-NLS-1$
        table.setColumnCollapsingAllowed(true);

        // use full date except if configured explicitly otherwise
        ...
        // hide "kg missing to meet 20kg rule" if the rule is not applied
        table.setColumnCollapsed("missingKg", !Competition.isEnforce15_20rule());
    ...
    public void displayClearAllConfirmation() {

        // Create the window...
        final Window subwindow = new Window(Messages.getString("WeighInList.deleteLifters", getApplication().getLocale())); //$NON-NLS-1$
        // ...and make it modal
        ...
        final Label message = new Label(
                Messages.getString("WeighInList.ClearAthletes", getApplication().getLocale()), Label.CONTENT_XHTML); //$NON-NLS-1$
        ...
        final Button ok = new Button(Messages.getString("WeighInList.deleteLifters", getApplication().getLocale()));
    ...
    public void displayResetAllConfirmation() {

        // Create the window...
        final Window subwindow = new Window(Messages.getString("WeighInList.resetLifters", getApplication().getLocale())); //$NON-NLS-1$
        // ...and make it modal
        subwindow.setModal(true);
        subwindow.setWidth("10cm"); //$NON-NLS-1$
        ...
        // Add some content; a label and a close-button
        final Label message = new Label(
                Messages.getString("WeighInList.ClearLifts", getApplication().getLocale()), Label.CONTENT_XHTML); //$NON-NLS-1$
        subwindow.addComponent(message);
        ...
        final Button ok = new Button(Messages.getString("WeighInList.resetLifters", getApplication().getLocale()));
        ok.addListener(new Button.ClickListener() { //$NON-NLS-1$
            private static final long serialVersionUID = 1L;
    ...
    protected boolean currentSessionSelected() {
        CompetitionSession currentCompetitionSession = app.getCurrentCompetitionSession();
    ...
    protected void resetAllLifters() {
        final List<Lifter> list = currentSessionliftersIfSelectedAllIfNot();
        for (Lifter curLifter : list) {
            String cj1Decl = curLifter.getCleanJerk1Declaration();
            String sn1Decl = curLifter.getSnatch1Declaration();
            boolean enforce = Competition.isEnforce15_20rule();
            try {
                if (enforce) {
                    Competition.setEnforce15_20rule(false);
                }
                ...
                curLifter.setSnatch1Declaration(sn1Decl);
            } finally {
                Competition.setEnforce15_20rule(enforce);
                curLifter.setCleanJerk1Declaration(cj1Decl);
            }
    ...
    protected String[] getColHeaders() {
        ...
        if (registration) {
            COL_HEADERS = new String[] { Messages.getString("Lifter.membership", locale), //$NON-NLS-1$
                    ...
                    Messages.getString("Lifter.missingKg", locale), //$NON-NLS-1$
    ...
    public Object newItem() {
        ...
        final CompetitionSession currentGroup = app.getCurrentCompetitionSession();
    ...
    public String getFragment() {
        CompetitionSession session = app.getCurrentCompetitionSession();
        String sessionName = null;
        if (session != null) {
            sessionName = session.getName();
        }
        return viewName + "/" + (sessionName == null ? "" : sessionName);
    }
