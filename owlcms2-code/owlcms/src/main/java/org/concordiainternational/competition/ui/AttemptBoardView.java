        ...
        Stylable
{
    ...
    public AttemptBoardView(boolean initFromFragment, String fragment, String viewName, boolean publicFacing, String stylesheetName, CompetitionApplication app) {
    ...
    protected void createDecisionLights() {
        logger.trace("entry");
        decisionLights = new DecisionLights(publicFacing, app, false);
    ...
    private SessionDataUpdateEventListener registerAsListener(final String platformName1, final SessionData masterData1) {
        logger.trace("entry");
    ...
    private void displayAttemptNumber(Lifter lifter, final Locale locale, final int currentTry, boolean done) {
        ...
        if (!done) {
            final String lift = lifter.getAttemptsDone() >= 3 ? Messages.getString("AttemptBoard.shortCleanJerk", locale) //$NON-NLS-1$
                    : Messages.getString("AttemptBoard.shortSnatch", locale);//$NON-NLS-1$
    ...
    private void displayRequestedWeight(Lifter lifter, final Locale locale, boolean done) {
        ...
        if (!done && lifter != null) {
            Integer nextAttemptRequestedWeight = lifter.getNextAttemptRequestedWeight();
            if (nextAttemptRequestedWeight > 0) {
                weightLabel.setValue(nextAttemptRequestedWeight + Messages.getString("Common.kg", locale)); //$NON-NLS-1$
            } else {
                weightLabel.setValue("");
            }
        } else {
    ...
    private void displayIntermissionTimer(Integer remainingMilliseconds) {
        ...
        synchronized (app) {
            ...
            CompetitionApplication current = CompetitionApplication.getCurrent(); // weird, double check this
            SessionData masterData2 = current.getMasterData();
            Locale locale = CompetitionApplication.getCurrentLocale();

            Lifter currentLifter = null;
            if (masterData2 != null) {
                currentLifter = masterData2.getCurrentLifter();
            }

            CompetitionSession currentCompetitionSession = current.getCurrentCompetitionSession();
            nameLabel.setValue(formatSession(locale, currentCompetitionSession));
            ...
            if (masterData2 != null) {
                ...
                displayRequestedWeight(currentLifter, locale, false);
            } else {
                showPlates(false);
                weightLabel.setValue("");
            }
    ...
    private void showPlates(boolean visible) {
        if (plates != null) {
            plates.setVisible(visible);
        }
    }
...
//            }
        ...
        refereeActionDispatcher = new RefereeActionDispatcher(masterData, app.getMainWindow());
