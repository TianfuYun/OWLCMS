...
import org.concordiainternational.competition.tests.AbstractLifterComparatorTest;
...
public class AbstractLifterComparator {
    ...
    int compareRegistrationCategory(Lifter lifter1, Lifter lifter2) {
        ...
        if (lifter1Category == null && lifter2Category == null)
            return 0;
        if (lifter1Category == null)
            return -1;
        if (lifter2Category == null)
            return 1;
        ...
        // registration categories sort by name because several can share
        // same weight boundaries
        String name1 = lifter1.getRegistrationCategory().getName();
        String name2 = lifter2.getRegistrationCategory().getName();
        // keep only the starting part
        name1 = getCategoryPrefix(name1);
        name2 = getCategoryPrefix(name2);
    ...
    int compareGroupWeighInTime(Lifter lifter1, Lifter lifter2) {
        ...
        if (lifter1String == null && lifter2String == null)
            return 0;
        if (lifter1String == null)
            return -1;
        if (lifter2String == null)
            return 1;
        return lifter1String.compareTo(lifter2String);
    }

    /**
     * Comparer les totaux des leveurs, si ils ont termin� tous leurs essais. Le leveur ayant terminé va après, de manière à ce le premier à
     * lever soit toujours toujours le premier dans la liste.
     *
     * @param lifter1
     * @param lifter2
    ...
    /**
     * Compare birth dates.
     * No birth date implies newborn.
     * @param lifter1
     * @param lifter2
     * @return 1 if lifter1 is younger; 0 if equal, -1 if older.
     */
    int compareBirthDate(Lifter lifter1, Lifter lifter2) {
        Date lifter1Value = lifter1.getFullBirthDate();
        Date lifter2Value = lifter2.getFullBirthDate();
    ...
    /**
     * Compare absolute value of attempts prior to attempt "startingFrom" Start comparing attempted weights at "startingFrom". If attempted
     * weight differ, smallest attempted weight comes first. If attempted weights are same, go back one attempt and keep comparing.
     *
