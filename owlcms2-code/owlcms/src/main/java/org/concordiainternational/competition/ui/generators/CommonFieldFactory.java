...
public class CommonFieldFactory extends DefaultFieldFactory {
     ...
     * (non-Javadoc)
     *
     * @see com.vaadin.ui.DefaultFieldFactory#createField(com.vaadin.data.Container, java.lang.Object, java.lang.Object,
    ...
    public static Field createFieldByPropertyType(Class<?> type, Component uiContext) {
        ...
        if (Category.class.isAssignableFrom(type)) {
            return new CategorySelect(CompetitionApplication.getCurrent());// getCurrent() required because of static scope
        }

        if (Platform.class.isAssignableFrom(type)) {
            return new PlatformSelect(CompetitionApplication.getCurrent());// getCurrent() required because of static scope
        }
    ...
    private Field checkInteger(final Field f) {
        ...
        f.addValidator(new Validator() {
            ...
            public void validate(Object value) throws InvalidValueException {
                if (!isValid(value))
                    throw new InvalidValueException(Messages.getString("CommonFieldFactory.badNumberFormat", app.getLocale())); //$NON-NLS-1$
            }
        });
        return f;
    }
