...
public class AnnouncerView extends VerticalSplitPanel implements
        ApplicationView,
        SessionData.SessionDataUpdateEventListener,
        EditingView,
        Notifyable,
        Window.CloseListener
{
    ...
    protected static final boolean UPDATE_BOTTOM = true;
     ...
     * @param mode
     */
    public AnnouncerView(boolean initFromFragment, String fragment, String viewName, Mode mode, CompetitionApplication app) {
        ...
        if (mode == Mode.ANNOUNCER) {
            ...
            masterData.setMasterApplication(app);
        ...
        announcerInfo = new LifterInfo("topPart", app, masterData, mode, this); //$NON-NLS-1$
        ...
        liftList = new LiftList(masterData, this, mode, app);
...
//            announcerInfo.setWidth("36ex");
    ...
    public void loadFirstLifterInfo(SessionData groupData) {
        ...
        if (WebApplicationConfiguration.isAutoLoadLifterEditor() && mode != Mode.MARSHALL) {
    ...
    private void updateLifterEditor(final Lifter lifter, final Item lifterItem) {
        ...
        if (lifter != null) {
            if (lifterCardEditor == null) {
                logger.trace(this.instanceId+" creating in app "+app);
                lifterCardEditor = new LifterCardEditor(app, liftList, this);
                lifterCardEditor.loadLifter(lifter, lifterItem);
                this.setSecondComponent(lifterCardEditor);
            } else {
                logger.trace(this.instanceId+" loading");
                lifterCardEditor.loadLifter(lifter, lifterItem);
            }
            ...
            public void run() {
                synchronized (app) {
                    if (sessionDataUpdateEvent.getForceRefresh()) {
                        logger.trace("updateEvent() received in {} view -- forced refresh. ----------------------------------", mode); //$NON-NLS-1$
                        if (mode == Mode.MARSHALL && lifterCardEditor != null) {
                            logger.debug("marshallMode: clearing lifterEditor");
                        ...
                        // if the announcer clicked clicked announce, the current lifter is still
                        // the same and we don't want to change the bottom part (as this would cause
                        // the marshall to lose focus if the current lifter was being edited).  In theory
                        // the marshall would have used "weight change" if the current lifter is being
                        // edited, but in practice this may well not happen.
                        if (!sessionDataUpdateEvent.isAnnounce()) {
                            loadFirstLifterInfo(masterData,
                                    WebApplicationConfiguration.DEFAULT_STICKINESS);
    ...
    public void switchGroup(final CompetitionSession newSession) {
        ...
        } else if (mode == Mode.MARSHALL && lifterCardEditor != null) {
            logger.debug("marshallMode: clearing lifterEditor");
            lifterCardEditor.unregisterAsListener();
            lifterCardEditor = null;
        }
