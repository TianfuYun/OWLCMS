...
public class LifterInfo extends VerticalLayout implements
        ...
        ApplicationView,
        Notifyable, SessionDataUpdateEventListener
{
    ...
    public LifterInfo(String identifier, CompetitionApplication app, final SessionData groupData, AnnouncerView.Mode mode, EditingView parentView) {
        ...
        synchronized (app) {
            ...
            if (lifter1 != null) {
                if (lifter1 == prevLifter
                        && ObjectUtils.compare(lifter1.getAttemptsDone(),prevAttempt) == 0
                        && ObjectUtils.compare(lifter1.getNextAttemptRequestedWeight(),prevWeight) == 0) {
                    return; // we are already showing correct information.
                }
                if (isTop()) {
                    lifter1.checkStartingTotalsRule(false, (Notifyable) parentView);
                }
                prevLifter = lifter1;
    ...
    private void topDisplayOptions(final Lifter lifter1, final SessionData groupData1) {
        ...
        timerControls = new TimerControls(lifter1, groupData1, true, mode, this, showTimerControls, app, parentView);
    ...
    public void showInteractionNotification(CompetitionApplication originatingApp, InteractionNotificationReason reason) {
        ...
        // Some notifications can arise from announcer errors (especially when keeping time) and should be shown.
        ...
        if (isTop() && (receivingApp != originatingApp || announcerShouldSee)) {
            ...
            ApplicationView receivingView = receivingApp.components.currentView;

            ApplicationView originatingAppView = originatingApp.components.currentView;
            //logger.debug("{} received notification from {}: {}", receivingView, originatingAppView, reason);

            if (receivingView instanceof AnnouncerView && reason != InteractionNotificationReason.UNKNOWN) {
                AnnouncerView receivingAnnouncerView = (AnnouncerView)receivingView;
                if (originatingAppView instanceof AnnouncerView) {
                    AnnouncerView originatingView = (AnnouncerView) originatingAppView;
                    if (originatingView.mode != receivingAnnouncerView.mode
                            && originatingView.mode != Mode.TIMEKEEPER) {
                        traceBack(originatingApp);
                        receivingAnnouncerView.displayNotification(originatingView.mode, reason);
                    } else if (announcerShouldSee) {
                        receivingAnnouncerView.displayNotification(originatingView.mode, reason);
                    }
                } else {
                    traceBack(originatingApp);
                    receivingAnnouncerView.displayNotification(null, reason);
                }
            }
        }
    }
    ...
    private void traceBack(CompetitionApplication originatingApp) {
        ...
        if (logger.isTraceEnabled()) {
            ...
            LoggerUtils.infoException(logger, new Exception("Stack Trace"));
        }
    }
    ...
    public void registerAsListener() {
        ...
        if (timer != null && !isBottom() && !(timer.getMasterBuzzer() == this)) {
            ...
            listenerLogger.debug("window: {} timer listener for {} {}",
                    new Object[] { mainWindow, identifier, this });
        }
    ...
    public void sessionDataUpdateEvent(SessionDataUpdateEvent sessionDataUpdateEvent) {
        ...
        Lifter currentLifter = sessionDataUpdateEvent.getCurrentLifter();
        if (currentLifter == clockOwner) {
            // lifter owns clock, start with remaining time.
            int timeRemaining = groupData.getTimeRemaining();
            logger.debug("currentLifter = {}, clockOwner = {}, timeRemaining = {}", currentLifter,clockOwner,timeRemaining );
            setTimerDisplay(timeRemaining);
        } else {
            int timeAllowed = groupData.getTimeAllowed();
            logger.debug("*** currentLifter = {}, clockOwner = {}, timeAllowed = {}", currentLifter,clockOwner,timeAllowed );
            setTimerDisplay(timeAllowed);
        }
    }
