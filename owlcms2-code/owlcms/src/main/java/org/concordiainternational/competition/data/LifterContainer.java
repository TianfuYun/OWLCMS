...
public class LifterContainer extends HbnContainer<Lifter> {
    ...
    public Criteria addSearchCriteria(Criteria criteria) {
        createCompetitionSessionCriteria(((CompetitionApplication) app).getCurrentCompetitionSession(), criteria);
    ...
    public static void createCompetitionSessionCriteria(CompetitionSession currentGroup, Criteria criteria) {
        final String name = (currentGroup != null ? (String) currentGroup.getName() : null);
        if (currentGroup != null) {
            // in case we refactor the name of competition sessions again...
            ...
            String tableName = simpleName.replace(firstChar, nFirstChar);

            // create the actual criterion on the linked table
            criteria.createCriteria(tableName).add(
