...
public class IntermissionTimer implements Serializable {
     ...
     */
    public void start() {
        logger.debug("enter start {}", getRemainingSeconds()); //$NON-NLS-1$
        if (timer != null) {
            timer.cancel();
        }
...
//        }
        ...
        countdownTask = null;
        remainingSeconds = 0;
        logger.debug("cleared timer"); //$NON-NLS-1$
        fireTimerEvent(0);
    }
    ...
    private void fixRemainingSeconds() {
        if (countdownTask != null) {
	 ...
	 *
	 */
    private void fireTimerEvent(int newRemainingSeconds) {
        IntermissionTimerEvent timerEvent = new IntermissionTimerEvent();
        this.remainingSeconds = newRemainingSeconds;
        timerEvent.setRemainingMilliseconds(newRemainingSeconds * 1000);
...
//        this.endTime = new Date(System.currentTimeMillis() + (i * 1000));
        logger.trace("before requested = {}  remaining = {} real = {}", requestedSeconds, remainingSeconds, getRemainingSeconds() );
        fireTimerEvent(requestedSeconds);
        logger.trace("after requested = {}  remaining = {} real = {}", requestedSeconds, remainingSeconds, getRemainingSeconds() );
    }
    ...
    public void setEndTime(Date endTime) {
        ...
        int deltaSeconds;
        if (deltaMillis > 0) {
            deltaSeconds = (int) Math.round(deltaMillis / 1000D);
        } else {
            deltaSeconds = 0;
        }
        fireTimerEvent(deltaSeconds);
    }
    ...
    public int getRemainingSeconds() {
        if (isRunning()) {
            return (int) countdownTask.getBestTimeRemaining() / 1000;
        } else {
