...
public class CommonColumnGenerator implements Table.ColumnGenerator {
    ...
    public Component generateCell(Table table, Object itemId, Object propertyId) {
        ...
        } else if (propertyIdString.equals("missingKg")) { //$NON-NLS-1$
            return generateMissingKgCell(table, itemId, item, propertyIdString);
        } else {
     ...
     * @return
     */
    private Component generateMissingKgCell(Table table, Object itemId, final Item item, final String propertyIdString) {
        Component c = null;
        if (item instanceof EntityItem) {
            c = dynamicMissingKgLabel(table, item, itemId, propertyIdString);
        } else {
            c = entryMissingKgLabel(table, item, itemId, propertyIdString);
        }
        ...
        public String format(Object value) {
            ...
            try {
                ...
                if (rank == 0) {
                    ...
                    return Messages.getString("CommonColumnGenerator.InvitedAbbreviation", CompetitionApplication.getCurrentLocale()); //$NON-NLS-1$
    ...
    /**
     * Selection list when multiple categories need to be selected (e.g. which categories belong to the same group)
     *
     * @param itemId
     * @return
     */
    ...
    /**
     * Selection when only one category is relevant (e.g. when entering registration category for a lifter)
     *
     * @param itemId
     * @param prop
     * @return
...
//                logger.debug("received event for " + lifter1 + " update of " + updateEvent.getPropertyIds());
     ...
     * @return
     */
    private Component dynamicMissingKgLabel(Table table, final Item item, Object itemId, Object propertyId) {
        final Lifter lifter = getLifter(item);

        final Label missingKgLabel = new Label("-"); //$NON-NLS-1$
        setEntryTotalLabel(lifter, missingKgLabel);
        ...
        final Lifter.UpdateEventListener listener = new Lifter.UpdateEventListener() {
            ...
            public void updateEvent(Lifter.UpdateEvent updateEvent) {
                Lifter lifter1 = (Lifter) updateEvent.getSource();
                setEntryTotalLabel(lifter1, missingKgLabel);
                app.push();
            }

        };
        if (missingKgLabel.getData() == null) {
            lifter.addListener(listener);
            missingKgLabel.setData(listener);
        }

        // done
        return missingKgLabel;
    }

    private int getDelta(int startingTotal, final Lifter lifter) {
        int delta = lifter.getQualifyingTotal() - startingTotal;
        return delta;
    }

    private int startingTotal(final Lifter lifter) {
        Integer snatchRequest = Lifter.last(
                Lifter.zeroIfInvalid(lifter.getSnatch1Declaration()),
                Lifter.zeroIfInvalid(lifter.getSnatch1Change1()),
                Lifter.zeroIfInvalid(lifter.getSnatch1Change2()));
        Integer cleanJerkRequest = Lifter.last(
                Lifter.zeroIfInvalid(lifter.getCleanJerk1Declaration()),
                Lifter.zeroIfInvalid(lifter.getCleanJerk1Change1()),
                Lifter.zeroIfInvalid(lifter.getCleanJerk1Change2()));

        int curStartingTotal = snatchRequest + cleanJerkRequest;
        return curStartingTotal;
    }
     ...
     * @return
     */
    private Component entryMissingKgLabel(Table table, final Item item, Object itemId, Object propertyId) {
        final Lifter lifter = getLifter(item);
        final Label entryTotalLabel = new Label("-"); //$NON-NLS-1$
        setEntryTotalLabel(lifter, entryTotalLabel);
        return entryTotalLabel;
    }
    ...
    /**
     * Selection when only one platform is relevant (e.g. when entering registration platform for a lifter)
     *
     * @param itemId
     * @param prop
     * @return
    ...
    public Component getPlatformPopUpFor(Object itemId, final Property prop) {
        ...
        if (platformSelect == null) {
            ...
            platformSelect.setRequiredError(Messages.getString("CommonFieldFactory.youMustSelectAPlatform", app.getLocale())); //$NON-NLS-1$
    ...
    public Field getCategoryComboboxFor(Object itemId) {
        ...
        if (categorySelect == null) {
            ...
            categorySelect.setRequiredError(Messages.getString("CommonFieldFactory.youMustSelectACategory", app.getLocale())); //$NON-NLS-1$
            categorySelect.setWriteThrough(true);
            categorySelect.setImmediate(true);
    ...
    private void setEntryTotalLabel(final Lifter lifter1, final Label declaredTotalLabel) {
        int startingTotal = startingTotal(lifter1);
        int delta1 = getDelta(startingTotal,lifter1);
        final Integer entryTotal = lifter1.getQualifyingTotal();
        synchronized (app) {
            declaredTotalLabel.removeStyleName("wrong"); //$NON-NLS-1$
            if (entryTotal == null || entryTotal <= 0) {
                declaredTotalLabel.setValue("-");
            } else if (startingTotal <= 0) {
                declaredTotalLabel.setValue(entryTotal.toString());
            } else if (entryTotal != null) {
                int _20kgRuleValue = lifter1.get20kgRuleValue();
                if (delta1 > _20kgRuleValue) {
                    declaredTotalLabel.setValue((delta1-_20kgRuleValue));
                    declaredTotalLabel.addStyleName("wrong"); //$NON-NLS-1$
                } else {
                    declaredTotalLabel.setValue("-");
                }
            }
