...
public class TimerControls extends GridLayout {
    ...
    public TimerControls(final Lifter lifter, final SessionData groupData, boolean top, AnnouncerView.Mode mode,
            LifterInfo lifterInfo, boolean timerVisible, CompetitionApplication app, EditingView parentView) {
    ...
    public void enableButtons(SessionData groupData, String whereFrom) {
        ...
        logger.trace("synchronizing on app = {}",System.identityHashCode(app));
        synchronized (app) {
            if (!running) {
                if (announced) {
        ...
        final Button.ClickListener changeWeightListener = new Button.ClickListener() { //$NON-NLS-1$
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (mode == Mode.ANNOUNCER || mode == Mode.MARSHALL) {
                    AnnouncerView announcerView = (AnnouncerView)app.components.currentView;
    ...
    public void startDoIt(final Lifter lifter, final SessionData groupData) {
        ...
        if (groupData.getTimer().isRunning()) {
            ...
            IDecisionController refereeDecisionController = groupData.getRefereeDecisionController();
            refereeDecisionController.initDownSignal();  // force reset of sound if changed from configuration screen
            refereeDecisionController.setBlocked(false);
