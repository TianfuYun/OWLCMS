...
public class RefereeDecisions extends VerticalLayout implements DecisionEventListener, ApplicationView {
    ...
    public RefereeDecisions(boolean initFromFragment, String fragment, String viewName, boolean publicFacing, boolean juryMode, boolean listenToKeys, CompetitionApplication app) {
        this.app = app;
        if (initFromFragment) {
    ...
    public RefereeDecisions(boolean initFromFragment, String fragment, String viewName, boolean publicFacing, CompetitionApplication app) {
        this(initFromFragment, fragment, viewName, publicFacing, false, false, app);
        registerAsListener();
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                synchronized (app) {
                    ...
                    case DOWN:
                        logger.debug("received DOWN event dontShow={}", juryMode);
                        showLights(decisions, false, juryMode);
                        break;
                    case WAITING:
                        logger.debug("received WAITING event dontShow={}", juryMode);
                        showLights(decisions, false, juryMode);
                        break;
                    case UPDATE:
                        logger.debug("received UPDATE event dontShow=({] && {})", juryMode, !shown);
                        showLights(decisions, false, (juryMode && !shown));
                        break;
                    case SHOW:
                        logger.debug("received SHOW event dontShow={}", false);
