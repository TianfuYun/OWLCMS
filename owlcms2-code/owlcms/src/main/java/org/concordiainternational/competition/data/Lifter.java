...
public class Lifter implements MethodEventSource, Notifier {
    ...
    static public List<Lifter> getAll(boolean excludeNotWeighedIn) {
        Session hbnSession = null;
        try {
            hbnSession = CompetitionApplication.getCurrent().openSession();
            List<Lifter> lifters = getAll(hbnSession, excludeNotWeighedIn);
            return lifters;
        } finally {
            if (hbnSession != null) {
                hbnSession.clear();
    ...
    static public List<Lifter> getAllCurrentGroup(CompetitionSession session, boolean excludeNotWeighedIn) {
        Session hbnSession = null;
        try {
            hbnSession = CompetitionApplication.getCurrent().openSession();
            List<Lifter> lifters = getAllCurrentGroup(hbnSession, session, excludeNotWeighedIn);
            return lifters;
        } finally {
            if (hbnSession != null) {
                hbnSession.clear();
    ...
    static public List<Lifter> getAllParticipants() {
        Session hbnSession = null;
        try {
            hbnSession = CompetitionApplication.getCurrent().openSession();
            List<Lifter> lifters = getAllParticipants(hbnSession);
            return lifters;
        } finally {
            if (hbnSession != null) {
                hbnSession.clear();
    ...
    @SuppressWarnings("unchecked")
    static public List<Lifter> getAllCurrentGroup(Session hbnSession, CompetitionSession session, boolean excludeNotWeighedIn) {
        Criteria criteria = hbnSession.createCriteria(Lifter.class);
        if (session != null && session.getName() != null) {
            LifterContainer.createCompetitionSessionCriteria(session, criteria);
        }
        if (excludeNotWeighedIn) {
            criteria.add(Restrictions.gt("bodyWeight", 0.01D)); //$NON-NLS-1$
        }
        return criteria.list();
    }

    protected static List<Lifter> getAllParticipants(Session hbnSession) {
        List<Lifter> allLifters = getAll(hbnSession, false);
        List<Lifter> filteredLifters = new ArrayList<Lifter>(allLifters.size());
        for (Lifter curLifter: allLifters) {
            if (curLifter.getCompetitionSession() != null && curLifter.isATeamMember()) {
                filteredLifters.add(curLifter);
            }
        }
        return filteredLifters;
    }
    ...
    public String getMastersAgeGroupInterval() {
        ...
        if (this.getGender().equals("F") && ageGroup1 >= 70) { //$NON-NLS-1$
            return "70+";
        }
    ...
    public Double getSinclairFactor() {
        if (gender.equalsIgnoreCase("M")) { //$NON-NLS-1$
            return sinclairFactor(this.bodyWeight, Coefficients.menCoefficient(), Coefficients.menMaxWeight());
        } else {
            return sinclairFactor(this.bodyWeight, Coefficients.womenCoefficient(), Coefficients.womenMaxWeight());
        }
    }
...
//                ||
                return membership.equalsIgnoreCase(Messages.getString("Lifter.InvitedAbbreviated", locale)) //$NON-NLS-1$
        // || !getTeamMember()
        ;
    }
    ...
    public void checkStartingTotalsRule(boolean unlessCurrent) {
        ...
        if (current != null) {
            ...
            if (mainLayoutContent != null && mainLayoutContent instanceof Notifyable) {
                checkStartingTotalsRule(unlessCurrent, (Notifyable) mainLayoutContent);
            }
        }

    }

    public void checkStartingTotalsRule(boolean unlessCurrent, Notifyable parentView) throws RuleViolationException {
...
//        if (!Competition.isMasters())
        {
            ...
            if (delta > _20kgRuleValue) {
                ...
                message = RuleViolation.rule15_20Violated(this.getLastName(), this.getFirstName(), (startNumber2 != null ? startNumber2 : "-") , snatchRequest,
...
//        }
    ...
    private String getMastersAgeGroup(String gender1) {
        ...
        String agePlus = "";
        if ("M".equals(gender1) && ageGroup1 == 80) agePlus = "+";
        else if ("F".equals(gender1) && ageGroup1 == 70) agePlus ="+";
        else if (ageGroup1 == 17) agePlus = "-";
        else if (ageGroup1 == 20) agePlus = "-";
        else if (ageGroup1 == 34) {
            ageGroup1 = 21;
        }
...
//                        : "category")), //$NON-NLS-1$
        if (WebApplicationConfiguration.isUseRegistrationCategory()) {
            return getRegistrationCategory().getName();
...
//    }
    ...
    public int get20kgRuleValue() {
        ...
        } else if (WebApplicationConfiguration.isUseOld20_15rule()) {
            if ("M".equals(this.getGender())) {
                return 20;
            } else {
                return 15;
