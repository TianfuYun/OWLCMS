...
public class CompetitionEditor extends VerticalLayout implements ApplicationView {
    ...
    public CompetitionEditor(boolean initFromFragment, String fragment, String viewName, CompetitionApplication app) {
        this.app = app;
...
//        addTextField(formLayout, locale, competitionItem, "invitedIfBornBefore", 0); //$NON-NLS-1$
        ...
        try {
            ...
            editPS = addFileSelector(formLayout, locale, competitionItem,
                    "protocolFileName", "", "templates/protocolSheet", displayPS);
        } catch (IOException e) {
        }
        ...
        try {
            ...
            editFN = addFileSelector(formLayout, locale, competitionItem,
                    "resultTemplateFileName", "", "templates/competitionBook", displayFN); //$NON-NLS-1$ //$NON-NLS-2$)
        } catch (IOException e) {
        }
    ...
    private Select addFileSelector(FormLayout formLayout, Locale locale,
            ...
            String relativeLocation, final TextField displayField) throws IOException {
        ...
        realPath = CompetitionApplication.getCurrent().getResourceFileName(relativeLocation);

        File file = new File(realPath);
        resourceLogger.debug("addFileSelector {} {}",file.getCanonicalPath(),file.isDirectory());
...
//    private String getPathFromWebContext(String relativeLocation, WebApplicationContext wContext) {
//        return wContext.getHttpSession().getServletContext().getRealPath("/WEB-INF/classes/"+relativeLocation);
//    }
    ...
    private FilesystemContainer findTemplatesWhenRunningInPlace(WebApplicationContext wContext) {
        ...
        try {
            resourceLogger.debug("findTemplatesWhenRunningInPlace 1 {}", realPath);
            // really ugly, no benefit whatsoever in cleaning this up.
            File file1 = new File(realPath).getParentFile().getParentFile();
            resourceLogger.debug("findTemplatesWhenRunningInPlace 2 {}", file1.getAbsolutePath());
            if (realPath != null && file1.isDirectory()) {
                file1 = new File(file1, "resources/templates/competitionBook");
                resourceLogger.debug("findTemplatesWhenRunningInPlace 3 {}", file1.getAbsolutePath());
