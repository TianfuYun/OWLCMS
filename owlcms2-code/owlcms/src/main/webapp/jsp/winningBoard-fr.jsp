...
<%  };
	...
	if (lifters == null || lifters.size() == 0) {
		out.println("</head><body>Il n'y a pas de groupe actuellement en compétition sur le plateau.</body></html>");
		out.flush();
		return;
	}
...
%>
<title>Classement - Ranking</title>
...
<c:choose>
	<c:when test='${useGroupName}'>
		<span class="title">Classement groupe ${groupName} &ndash; Group ${groupName} Ranks</span>
		<span class="liftsDone">${liftsDone}</span>
	</c:when>
	<c:otherwise>
		<span class="title">Classement &ndash; Ranks</span>
		<span class="liftsDone">${liftsDone}</span>
	</c:otherwise>
</c:choose>
</div>
